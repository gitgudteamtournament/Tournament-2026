...
import AdminCalendarOverlay from "./AdminCalendarOverlay";
...
const statusConfig: Record<TourStatus, { label: string; className: string }> = {
    draft: { label: "Draft", className: "bg-[#e2e8f0] text-[#475569]" },
    running: { label: "Running", className: "bg-[#dcfce7] text-[#16a34a]" },
    submission_closed: { label: "SubmissionClosed", className: "bg-[#fef9c3] text-[#854d0e]" },
    finished: { label: "Finished", className: "bg-[#f1f5f9] text-[#64748b]" },
};
...
type TabType = "main" | "scoring" | "leaderboard" | "calendar" | "certificate";

export default function TourProperty({ onClose, roundTitle, status = "draft" }: TourPropertyProps) {
    ...
    const menuItems: { id: TabType; label: string }[] = [
        ...
        { id: "calendar", label: "Розклад турніру" },
        ...
        <div className={Theme.overlay}>
            ...
            >
                <aside className={Theme.sidebar}>
                    <button
                        onClick={onClose}
                        className="hidden md:flex items-center gap-2 text-slate-400 hover:text-[#5c75ff] mb-6 transition-colors font-black text-[10px] uppercase tracking-[2px] bg-transparent border-none cursor-pointer"
                    >
                    ...
                    <div className="hidden md:flex flex-col gap-3 mb-6 px-4">
                        <h2 className="text-[20px] font-black text-[#1e293b]">
                            {roundTitle || "Раунд N: Назва"}
                        </h2>
                        <span className={`self-start text-[11px] font-bold px-3 py-1 rounded-full ${className}`}>
                            {label}
                        </span>
                    </div>
                ...
                <main className={Theme.contentArea}>
                    <AnimatePresence mode="wait">
                        ...
                        >
                            ...
                            {activeTab === "calendar" && <AdminCalendarOverlay onClose={() => setActiveTab("main")} />}
                        </motion.div>
                    </AnimatePresence>
                </main>
                ...
                >
                    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#1e293b" strokeWidth="3">
