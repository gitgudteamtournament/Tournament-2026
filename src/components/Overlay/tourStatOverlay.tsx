...
export default function TourStatOverlay({ onRegister }: { onRegister: () => void }) {
    return (
        <motion.main initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }} className="mt-8 space-y-6">
            <div className="rounded-[30px] bg-[#FFFFFF26] backdrop-blur-[35px] border border-white/40 p-8 md:p-12 shadow-sm">
                <div className="flex flex-col lg:flex-row justify-between gap-8">
                    <div className="flex-1">
                        ...
                        <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={onRegister} className="bg-[#5c75ff] text-white font-bold py-3 px-12 rounded-[12px] shadow-lg hover:brightness-110 transition-all">Продовжити</motion.button>
                    </div>
